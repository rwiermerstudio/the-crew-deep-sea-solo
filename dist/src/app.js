...
function render(){
 ...
 el.trick.replaceChildren(...state.trick.map((entry,i)=>{const d=document.createElement('div');d.className='played';d.style.setProperty('--tilt',`${(i-1.5)*5}deg`);d.innerHTML=`<div class="${cardClass(entry.card)}"><span>${SUIT_NAMES[entry.card.suit]}</span><b>${entry.card.value}</b><span>${entry.card.suit==='sub'?'TRUMP':'PRESSURE'}</span></div><small>${names[entry.player]}</small>`;return d;}));
 el.tasks.replaceChildren(...state.tasks.map(t=>{const d=document.createElement('div');d.className=`task ${t.done?'done':''} ${t.failed?'fail':''}`;d.textContent=`${t.done?'✓ ':t.failed?'× ':''}${cardLabel(t.card)} → ${names[t.owner]}`;return d;}));
}
